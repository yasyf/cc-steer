...
cc-pushback's default database location, the ``triage`` verdict table, and the
``training_pairs`` view — the pipeline's final deliverable.
"""
...
__all__ = ["FEEDBACK_DDL", "TRIAGE_DDL", "FeedbackStore", "Stats", "TriageStats", "event_row"]
...
SELECT
  e.id AS event_id,
  e.dedup_key,
  t.what_claude_did AS claude_action_summary,
  json_extract(e.context_json, '$.trigger.text') AS claude_action_text,
  json_extract(e.context_json, '$.trigger.tool_calls') AS claude_action_tools,
  json_extract(e.context_json, '$.trigger.tool_inputs') AS claude_action_tool_inputs,
  e.text AS pushback,
  t.category,
  t.confidence,
  t.prompt_version,
...
class FeedbackStore(BaseFeedbackStore):
    ...
    adds the ``triage`` verdict table plus the ``training_pairs`` view. Verdicts key
    ...
    async def open(cls, path: Path) -> Self:
        ...
        return cls(await FileStateStore.open(path, extra_schema=FEEDBACK_DDL + TRIAGE_DDL))
    ...
    async def pairs(self) -> list[dict[str, object]]:
        """Returns every row of the ``training_pairs`` view, the pipeline's deliverable."""
        cur = await self.store.conn.execute("SELECT * FROM training_pairs ORDER BY event_id")
        return [dict(row) async for row in cur]
